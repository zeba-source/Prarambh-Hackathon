(function (global, factory) {
    ...
    (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.ReactGoogleMaps = {}, global.React, global.ReactDOM, global.fastDeepEqual));
})(this, (function (exports, React, reactDom, isDeepEqual) { 'use strict';
    ...
    ***************************************************************************** */
    ...
    function useGoogleMapsApiLoader(props) {
        ...
        const [status, setStatus] = React.useState(GoogleMapsApiLoader.loadingStatus);
        const [loadedLibraries, addLoadedLibrary] = React.useReducer((loadedLibraries, action) => {
            return loadedLibraries[action.name]
                ? loadedLibraries
                : Object.assign(Object.assign({}, loadedLibraries), { [action.name]: action.value });
        }, {});
        const librariesString = React.useMemo(() => libraries === null || libraries === void 0 ? void 0 : libraries.join(','), [libraries]);
        const serializedParams = React.useMemo(() => JSON.stringify(Object.assign({ apiKey, version }, otherApiParams)), [apiKey, version, otherApiParams]);
        const importLibrary = React.useCallback((name) => __awaiter(this, void 0, void 0, function* () {
            var _a;
    ...
    function useInternalUsageAttributionIds(props) {
        const internalUsageAttributionIds = React.useMemo(() => props.disableUsageAttribution
            ? null
    ...
    const APIProvider = props => {
        ...
        return (React.createElement(APIProviderContext.Provider, { value: contextValue }, children));
    };
    ...
    function useApiLoadingStatus() {
        var _a;
        return ((_a = React.useContext(APIProviderContext)) === null || _a === void 0 ? void 0 : _a.status) || APILoadingStatus.NOT_LOADED;
    }
    ...
    function useMapInstance(props, context) {
        ...
        const savedMapStateRef = React.useRef(undefined);
    ...
    const Map = (props) => {
        ...
        const context = React.useContext(APIProviderContext);
        ...
        const combinedStyle = React.useMemo(() => (Object.assign({ width: '100%', height: '100%', position: 'relative', 
            // when using deckgl, the map should be sent to the back
            zIndex: isDeckGlControlled ? -1 : 0 }, style)), [style, isDeckGlControlled]);
        const contextValue = React.useMemo(() => ({ map }), [map]);
        if (loadingStatus === APILoadingStatus.AUTH_FAILURE) {
            return (React.createElement("div", { style: Object.assign({ position: 'relative' }, (className ? {} : combinedStyle)), className: className },
                React.createElement(AuthFailureMessage, null)));
        }
        return (React.createElement("div", Object.assign({ ref: mapRef, "data-testid": 'map', style: className ? undefined : combinedStyle, className: className }, (id ? { id } : {})), map ? (React.createElement(GoogleMapsContext.Provider, { value: contextValue }, children)) : null));
    };
    ...
    const AdvancedMarker = React.forwardRef((props, ref) => {
        const { children, style, className, anchorPoint } = props;
        const [marker, contentContainer] = useAdvancedMarker(props);
        const advancedMarkerContextValue = React.useMemo(() => (marker ? { marker } : null), [marker]);
        React.useImperativeHandle(ref, () => marker, [marker]);
        if (!contentContainer)
            return null;
        return (React.createElement(AdvancedMarkerContext.Provider, { value: advancedMarkerContextValue }, reactDom.createPortal(React.createElement(MarkerContent, { anchorPoint: anchorPoint, styles: style, className: className }, children), contentContainer)));
    });
    AdvancedMarker.displayName = 'AdvancedMarker';
    function useAdvancedMarkerRef() {
        const [marker, setMarker] = React.useState(null);
        const refCallback = React.useCallback((m) => {
            setMarker(m);
        }, []);
    ...
    const InfoWindow = props => {
        ...
        return (React.createElement(React.Fragment, null,
            contentContainerRef.current &&
                reactDom.createPortal(children, contentContainerRef.current),
            headerContainerRef.current !== null &&
                reactDom.createPortal(headerContent, headerContainerRef.current)));
    };
    ...
    const MapControl = ({ children, position }) => {
        const controlContainer = React.useMemo(() => document.createElement('div'), []);
        const map = useMap();
        React.useEffect(() => {
            if (!map)
                return;
    ...
    const Pin = props => {
        var _a;
        const advancedMarker = (_a = React.useContext(AdvancedMarkerContext)) === null || _a === void 0 ? void 0 : _a.marker;
        const glyphContainer = React.useMemo(() => document.createElement('div'), []);
